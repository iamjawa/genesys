...
function renderOrganismSVG(organism, size) {
  ...
  var CX = 50, CY = 50, petalDist = 27, petalR = 16, stemH = 18;
  ...
  for (var i = 0; i < 6; i++) {
    ...
    var px = CX + Math.cos(rad) * petalDist;
    var py = CY + Math.sin(rad) * petalDist;
    ...
    if (shape === 'Round') {
      ...
      var outer = petalR * 0.85, inner = outer * 0.4;
      var pts = '';
      for (var j = 0; j < 5; j++) {
        var a1 = (j * 72 - 90) * Math.PI / 180;
